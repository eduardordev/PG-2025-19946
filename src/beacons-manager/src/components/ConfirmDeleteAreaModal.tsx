...
export default function ConfirmDeleteAreaModal({ area, onConfirm, onCancel, loading = false }: ConfirmDeleteAreaModalProps) {
  if (!area) return null;
            ...
            <div className="mt-2 px-7 py-3">
              ...
              <div className="mt-3 bg-gray-700 rounded-lg p-3 text-left">
                <div className="text-sm">
                  <div className="font-medium text-white">{area.nombre}</div>
                  <div className="text-gray-300">Código: {area.codigo}</div>
                  <div className="text-gray-300">Número: {area.numero}</div>
                  <div className="text-gray-300">Ubicación: {area.ubicacion}</div>
                  {area.descripcion && (
                    <div className="text-gray-300">Descripción: {area.descripcion}</div>
                  )}
                </div>
              </div>
              <div className="mt-3 p-3 bg-yellow-900 bg-opacity-30 border border-yellow-600 rounded-lg">
                <p className="text-sm text-yellow-300">
                  ⚠️ Si esta área tiene sensores asociados, no se podrá eliminar.
                </p>
              </div>
            </div>
          </div>
